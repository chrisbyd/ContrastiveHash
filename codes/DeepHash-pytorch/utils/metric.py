...
class MAPs:
    ...
    def get_mAPs_by_feature(self, query_codes, query_labels, gallery_codes, gallery_labels, Rs=None, dist_type='inner_product'):
        if Rs is None:
            Rs = self.R
        return get_mAPs(query_codes, query_labels, gallery_codes, gallery_labels, Rs, dist_type)

    def get_mAPs_after_sign(self, query_codes, query_labels, gallery_codes, gallery_labels, Rs=None, dist_type='inner_product'):
        if Rs is None:
            Rs = self.R
    ...
    @staticmethod
    def get_precision_recall_by_Hamming_Radius(query_codes, query_labels, gallery_codes, gallery_labels, radius=2):
        # query_output = sign(query.output)
        # database_output = sign(database.output)

        bit_n = query_codes.shape[1] #bit number

        ips = np.dot(query_codes, gallery_codes.T)
        ips = (bit_n - ips) / 2 #
        ...
        for i in range(ips.shape[0]):
            ...
            if all_num != 0:
                imatch = np.sum(gallery_labels[idx[:], :] == label, 1) > 0
                match_num = np.sum(imatch)
                precX.append(np.float(match_num) / all_num)

                all_sim_num = np.sum(
                    np.sum(gallery_labels[:, :] == label, 1) > 0)
                recX.append(np.float(match_num) / all_sim_num)

                if radius < 10:
                    ips_trad = np.dot(
                        query_codes[i, :], gallery_codes[ids[i, 0:all_num], :].T)
                    ids_trad = np.argsort(-ips_trad, axis=0)
                    db_labels = gallery_labels[ids[i, 0:all_num], :]
    ...
    @staticmethod
    def get_precision_recall_by_Hamming_Radius_All(query_codes, query_labels, gallery_codes, gallery_labels):


        bit_n = query_codes.shape[1]
